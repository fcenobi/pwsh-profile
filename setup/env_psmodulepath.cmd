...
:getPathVariable
    ...
    echo.
    for /f "tokens=1,2,*" %%a in ( 'reg query "HKCU\Environment" /v PSMODULEPATH' ) do set usrpp=%%c
    for /f "tokens=1,2,*" %%a in ( 'reg query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v PSMODULEPATH' ) do set syspp=%%c
